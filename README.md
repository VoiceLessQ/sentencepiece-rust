...
- Output is verified against the Python `sentencepiece` oracle over thousands of
  real-text lines, both encode and decode. See
  [The Rust tokenizer test](#the-rust-tokenizer-test) and
  [Verification breadth](#verification-breadth). A difference from the oracle is
...
| v0.2 | Darts charsmap normaliser to full Unicode input | + Unicode corpora (full-width, ligatures, CJK, …) |
| v0.3 (current) | Unigram Viterbi segmentation; byte-fallback and byte-piece decode | 3 models (BPE, Unigram, BPE+byte_fallback), encode and decode |
...
directions are verified against the Python oracle.
...
2.3k Japanese lines) through all three models: 13,264 lines, encode and decode.
The result was 13,258 exact, 6 equal-score reorderings, and 0 real mismatches.

The 6 differences are all runs of repeated punctuation (`.......`, long `----`)
on the Unigram model. Each is a same-multiset reordering of pieces, that is, an
equally-optimal Viterbi path with the same total score. This is a documented
precision fence: the compiled oracle binary's optimised float arithmetic
resolves these exact ties differently than portable `f32`, and both
segmentations are optimal. See the note in [src/unigram.rs](src/unigram.rs). The
test counts these separately and only fails on a genuine (different-piece)
mismatch.

To reproduce the broad run, point `gen_oracle.py` at a larger corpus instead of
the small `corpus_*.txt` files. For example, use `botchan.txt` and
`wagahaiwa_nekodearu.txt` from the upstream SentencePiece repo's
...
[tests/oracle.rs](tests/oracle.rs) is a plain `cargo test` integration test. It
is pure Rust, with no Python at run time. For each fixture model it:
...
4. classifies any encode difference: a same-multiset reordering counts as an
   equally-optimal Viterbi tie (accepted, counted separately), and anything else
   is a genuine mismatch that fails the test.
...
The test self-skips green on a fresh checkout until the `oracle/cases*.tsv` files
exist, so `cargo test` never fails just because the oracle hasn't been generated
yet. The oracle (Python `sentencepiece`) is only used to produce those files; the
test itself, and the crate, need nothing but Rust.

## Speed

A rough throughput cross-check: [examples/bench.rs](examples/bench.rs) against
...
Read these as a sign that the port runs at a healthy native throughput, not as
proof that Rust beats C++. The Python column includes per-call CPython/FFI
overhead (the C++ core in batch mode would be faster), and this crate is
straightforward single-threaded inference tuned for matching the oracle rather
than for speed (for example, the normaliser re-runs `common_prefix_search` per
position). Even so, native Rust encode runs about 2 to 4 times the per-line
Python path, and decode about 5 to 7 times, with no native dependency to link.
